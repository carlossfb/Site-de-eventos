...
if(isset($_POST['save'])){
		...
		//Verificar se a data é valida ou se é do ano atual
							function validarData ($dia, $formato = 'Y-m-d H:i:s'){
										...
										if(!validarData($dia, 'Y-m-d') || date('Y-m-d', strtotime($dia)) < date('Y-m-d')){
											header("location:../novo_evento.php?erro=data invalida");
											die();
	}
	$categoria = 'musical, festivo';
	...
	//inserindo dados
	$sql = $conn->query(
	"INSERT INTO `tb_eventos` (`ddi_evento`,`nome_evento`,`categoria_evento`, `celular_evento`, `telefone_evento`,`dia_evento`, `inicio_evento`, `termino_evento`, `ingressoh`, `ingressom`, `obs_evento`, `email_evento`, `chek_evento`, `cep_evento`, `uf_evento`, `bairro_evento`, `rua_evento`, `cidade_evento`, `numero_evento`, `complemento_evento`, `id_postador`, `musicas`,`data_envio`,`visivel`)
	VALUES ('$ddi','$nome','$categoria', '$cel', '$tel', '$dia', '$inicio','$termino', '$ingressoh', '$ingressom', '$obs', '$email', '$check', '$cep', '$uf','$bairro','$rua','$cidade','$numero','$complemento','$post','$musicas','$data','true')");
